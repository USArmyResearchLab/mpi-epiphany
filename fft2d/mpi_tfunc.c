...
/* DAR
 * JAR - Added performance optimizations to fft_r2_dit, benchmarking loop
 */
...
void fft_r2_dit( unsigned int n, unsigned int m, void* p_wn, void* p_data )
{
	...
	for(l=1; l<m; l++) {
		...
		for(j=0; j<l1; j+=2) {
			cfloat wni0 = wn[i];
			cfloat wni1 = wn[i+wstride];
			i += 2*wstride;
			for(i0=j; i0<n; i0+=l2) {
				i1 = i0 + l1;
				cfloat x0 = data[i0];
				cfloat x1 = data[i0+1];
				cfloat y0 = wni0 * data[i1];
				cfloat y1 = wni1 * data[i1+1];
				data[i1] = x0 - y0;
				data[i1+1] = x1 - y1;
				data[i0] = x0 + y0;
				data[i0+1] = x1 + y1;
...
int loop;
for(loop=0; loop<nloop; loop++) { // synthetic timing loop
